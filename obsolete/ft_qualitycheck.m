...
% ensure that the cellfunction toolbox is on the path
...
if strcmp(cfg.analyze,'yes')
  ...
  if isctf
    try
      % update the info fields, this quite often does not work: FIXME
  ...
  timeaxis = (timeunit-timeunit/2):timeunit:(timeunit*ntrials-timeunit/2);
  ...
  if isctf % this fails for older CTF data sets
    Nx = match_str(info.hdr.label, 'HLC0011'); % x nasion coil
    Ny = match_str(info.hdr.label, 'HLC0012'); % y nasion
    Nz = match_str(info.hdr.label, 'HLC0013'); % z nasion
    Lx = match_str(info.hdr.label, 'HLC0021'); % x left coil
    Ly = match_str(info.hdr.label, 'HLC0022'); % y left
    Lz = match_str(info.hdr.label, 'HLC0023'); % z left
    Rx = match_str(info.hdr.label, 'HLC0031'); % x right coil
    Ry = match_str(info.hdr.label, 'HLC0032'); % y right
    Rz = match_str(info.hdr.label, 'HLC0033'); % z right
    headpos.dimord = 'chan_time';
    headpos.time   = timeaxis;
...
%     end
    ...
    tmp    = cellfun(@sum, abs(tmpdif) > jumpthreshold, tmpdimvec, 'uniformoutput', false);
